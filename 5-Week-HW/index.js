...
userData.forEach(id => {
  let phoneNumber = `Phone Number: ${id.phone}`;
  phoneNumberList.push(phoneNumber);
});
console.log(phoneNumberList);
//End of first bullet for Homework
const webEmail = [];
userData.forEach(id => {
  let info = `Email: ${id.email}: Website: ${id.website}`;
  webEmail.push(info);
});
console.log(webEmail);
//End of second bullet for Homework
...
//Third bullet point Homework done

const ninesIncompleteTasks = user9Tasks.filter(
  task => task.completed === false
);
console.log(ninesIncompleteTasks);
console.log(
  `User 9 has a total of ${ninesIncompleteTasks.length} incompleted tasks.`
);
//Fourth bullet point Homework done
let postData = [
  {
    userId: 1,
    id: 1,
...
//Sixth Bullet Point Homework Done
